Ext.define('Sales1.view.login.LoginPageController', {
						...
						if (jsonResponse.response.changePassword == true) {
							Ext.Msg.confirm('Confirm', jsonResponse.response.message, function(id, value) {
								if (id == 'yes') {
									...
									scope.container.removeAll();
									scope.container.add(Ext.create('Sales1.view.login.ChangePasswordScreen'));
								}
							}, {
								controller : this,
								scope : scope
		...
		Ext.Ajax.request({
			...
			success : function(response, sender) {
				...
				if (responseText.response.success) {
					...
					var forgetPasswordWindow = Ext.create('Sales1.view.login.ForgetPasswordWindow', {
